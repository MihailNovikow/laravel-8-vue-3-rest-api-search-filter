    ...
    <div class="overflow-hidden overflow-x-auto min-w-full align-middle sm:rounded-md">
        <div class="flex place-content-end mb-4">
            <div class="px-4 py-2 text-white bg-indigo-600 hover:bg-indigo-700 cursor-pointer">
                <router-link :to="{ name: 'ads.create' }" class="text-sm font-medium">Create ad</router-link>
            </div>
        </div>
        ...
        <table class="min-w-full border divide-y divide-gray-200">
            ...
            <template v-for="item in ads" :key="item.id">
                <tr class="bg-white">
                    ...
                    <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                        <router-link :to="{ name: 'ads.edit', params: { id: item.id } }"
                                     class="mr-2 inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150">
                            Edit
                        </router-link>
                        <button @click="deleteAd(item.id)"
...
export default {
    setup() {
        const { ads, getAds, destroyAd } = useAds()
